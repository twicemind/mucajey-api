...
function buildEditionSummaries(req, files) {
...
router.get('/', async (req, res) => {
  const doc = result.documentation({
    method: 'GET',
    path: '/stats',
    description: 'Collects aggregated statistics about cached editions and cards.'
  });
  const cards = req.dataCache.listAllCards();
  const files = req.dataCache.listFiles();
  const editions = buildEditionSummaries(req, files);
  ...
  for (const card of cards) {
    ...
    if (appleId) {
      summary.cards_with_apple_id += 1;
    }
    if (appleUri) {
      summary.cards_with_apple_uri += 1;
    }
    if (spotifyId) {
      summary.cards_with_spotify_id += 1;
    }
    if (spotifyUri) {
      summary.cards_with_spotify_uri += 1;
    }
    if (hasApple && hasSpotify) {
      summary.cards_with_both_streaming += 1;
    }
    if (hasApple || hasSpotify) {
    ...
    if (!card.id) {
      summary.cards_without_identifier += 1;
    }

    const language = (card.language_short || 'unknown').toLowerCase();
    summary.language_distribution[language] = (summary.language_distribution[language] || 0) + 1;
    ...
    summary.cards_per_edition[editionKey] = (summary.cards_per_edition[editionKey] || 0) + 1;
  }

  if (uniqueEdits.size > 0) {
    summary.average_cards_per_edition = Math.round((cards.length / uniqueEdits.size) * 100) / 100;
  }

  const message = result.message({
    docs: doc,
    message: 'Statistics aggregated from the cached editions.',
    data: {summary: summary, editions: editions}
  });
