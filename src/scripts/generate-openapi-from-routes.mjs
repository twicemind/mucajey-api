...
const routesDir = resolveCliPath(cliRoutesDir) || path.join(__dirname, '..', 'routes');
const outDir = resolveCliPath(cliOutDir) || path.join(__dirname, '..', 'openapi', 'paths');
  ...
  traverse(ast, {
    CallExpression(callPath) {
      ...
      const isResultDocumentation =
        ...
        (
          (callee.property.type === 'Identifier' && callee.property.name === 'documentation') ||
          (callee.property.type === 'StringLiteral' && callee.property.value === 'documentation')
        );
...
function main() {
  ...
  for (const fullPath of files) {
    ...
    if (Object.keys(paths).length === 0) {
      console.warn(`⚠️  Keine result.documentation()-Aufrufe in ${relative} gefunden.`);
      continue;
    }

    const yamlObj = { paths };
    const outFileName = path.basename(fullPath, path.extname(fullPath)) + '.yaml';
    const outPath = path.join(outDir, outFileName);

    fs.writeFileSync(outPath, yaml.dump(yamlObj), 'utf8');
