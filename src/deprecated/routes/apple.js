...
const docDefinition = {
  ...
  path: '/apple/login',
  description:
    'Issue or return an API key for apps authenticated via Sign in with Apple.',
};

router.post('/login', async (req, res) => {
  const doc = result.documentation(docDefinition);
  const { appleUserId, deviceId, appName, appVersion, platform } =
    req.body || {};

  if (!appleUserId) {
    return res
      .status(400)
      .json(result.error({ docs: doc, error: 'appleUserId is required' }));
  }
  if (!deviceId) {
    return res
      .status(400)
      .json(result.error({ docs: doc, error: 'deviceId is required' }));
  }
  if (!appName) {
    return res
      .status(400)
      .json(result.error({ docs: doc, error: 'appName is required' }));
  }

  const timestamp = new Date().toISOString();
  const existing = apiKeysStore.keys.find(
    entry =>
      entry.appleUserId === appleUserId &&
